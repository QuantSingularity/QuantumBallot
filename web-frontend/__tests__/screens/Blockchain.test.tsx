...
vi.mock("@/tables/transactions_table/page", () => ({
  default: () => (
    <div data-testid="table-transactions">Transactions Table Component</div>
  ),
}));

vi.mock("@/tables/pending_transactions_table/page", () => ({
  default: () => (
    <div data-testid="table-pending-transactions">
...
describe("Blockchain Component", () => {
  ...
  it("renders all blockchain components", () => {
    ...
    expect(
      screen.getByTestId("table-pending-transactions"),
    ).toBeInTheDocument();
  ...
  it("calls mine blocks API when Mine Block button is clicked", async () => {
    ...
    await waitFor(() => {
      expect(mockGet).toHaveBeenCalledWith(
        "http://localhost:3010/api/blockchain/mine",
      );
      expect(mockGet).toHaveBeenCalledWith(
        "http://localhost:3011/api/blockchain/mine",
      );
    });
  });
