...
describe("Login component", () => {
  it("Renders custom Login Button", () => {
    const { getByRole } = render(
      _jsx(BrowserRouter, { children: _jsx(Login, {}) }),
    );
...
describe("Candidates component", () => {
  it("Renders all buttons and labels correctly", () => {
    ...
    const loadCandidatesNotDeployedButton = screen.getByText(
      /Load Candidates \[Not Deployed\]/i,
    );
...
describe("Users component", () => {
  ...
  it("Renders user management section", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /Load \/ Refresh Users/i }),
    ).toBeInTheDocument();
