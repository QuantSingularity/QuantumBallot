...
export const UltimateSpeech = async (results: Results) => {
  ...
  const winnerResult: CandidateResult | undefined =
    results.candidatesResult.find(
      (x) => x.candidate.name === results.winner.name,
    );
  if (!winnerResult) {
  ...
  const str = `
        ...
        ${results.candidatesResult
          .filter((x) => x.candidate.name !== winnerResult.candidate.name)
          .map(
            (cr) =>
              `${cr.candidate.name} from ${cr.candidate.party} received ${cr.percentage}% of the votes, totaling ${cr.numVotes} vote${cr.numVotes > 1 ? "s" : ""}.`,
          )
          .join(" ")}
